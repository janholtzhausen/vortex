...
int tc_egress_tarpit(struct __sk_buff *skb)
{
    ...
    if (eth_proto == ETH_P_IP) {
        ...
        if (ip_hlen < 20 || (void *)ip + ip_hlen > data_end) return 0;
        ...
        // Validate TCP header
        if (tcp->doff < 5 || (void *)tcp + tcp->doff * 4 > data_end) return 0;
    ...
    if (eth_proto == ETH_P_IPV6) {
        ...
        if ((void *)(tcp6 + 1) > data_end) return 0;

        // Validate TCP header
        if (tcp6->doff < 5 || (void *)tcp6 + tcp6->doff * 4 > data_end) return 0;
