...
static inline int vortex_memcpy(void *dst, size_t dst_sz, const void *src, size_t n)
{
...
#define explicit_bzero(p, n)                                                                       \
    do {                                                                                           \
        volatile unsigned char *_p = (volatile unsigned char *)(p);                                \
        size_t _n = (n);                                                                           \
        while (_n--)                                                                               \
            *_p++ = 0;                                                                             \
    } while (0)
...
static inline uint64_t xxhash64(const void *data, size_t len)
{
    ...
    if (len >= 32) {
        ...
        h64 = (v1 << 1 | v1 >> 63) + (v2 << 7 | v2 >> 57) + (v3 << 12 | v3 >> 52) +
              (v4 << 18 | v4 >> 46);
        h64 ^= (v1 * PRIME2);
