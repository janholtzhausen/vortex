...
#define CONN_FLAG_BACKEND_POOLED                                                                   \
    (1 << 9) /* backend fd is keep-alive, return to pool after response */
...
struct __attribute__((aligned(64))) conn_hot {
    ...
    uint8_t ct_compressible; /* 1 = Content-Type is compressible (text/html, json, etc.); set on
                                first RECV_BACKEND chunk; used to gate splice */
    uint8_t
        zc_notif_count; /* in-flight send_zc notifications pending (see VORTEX_OP_SEND_CLIENT_ZC) */
    uint8_t _pad2[22];
};
...
static inline struct conn_hot *conn_hot(struct conn_pool *p, uint32_t id)
{
    return &p->hot[id];
}
static inline struct conn_cold *conn_cold_ptr(struct conn_pool *p, uint32_t id)
{
    return &p->cold[id];
}
static inline uint8_t *conn_recv_buf(struct conn_pool *p, uint32_t id)
{
    return p->recv_bufs[id];
}
static inline uint8_t *conn_send_buf(struct conn_pool *p, uint32_t id)
{
    return p->send_bufs[id];
}
