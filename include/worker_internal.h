...
#define RECV_WINDOW_GROW(h, n, buf_size)                                                           \
    do {                                                                                           \
 ...
 */
#define PREP_RECV(w, sqe, fd, slot, buf, len, _io_flags, buf_idx)                                  \
    do {                                                                                           \
...
#define PREP_SEND(w, sqe, fd, slot, buf, len, _io_flags, buf_idx)                                  \
    do {                                                                                           \
        int _pfd = ((w)->uring.files_registered) ? (slot) : (fd);                                  \
        if ((w)->uring.bufs_registered)                                                            \
...
void cache_chunked_store(struct worker *w, uint32_t cid, struct conn_hot *h,
