    ...
    if (g_fmt == LOG_FMT_JSON) {
        ...
        llen =
            snprintf(linebuf, sizeof(linebuf),
                     ...
                     timebuf, ts.tv_nsec / 1000000, level_str[level], event ? event : "", escaped);
    } else {
        llen = snprintf(linebuf, sizeof(linebuf), "%s.%03ldZ [%-5s] %s: %s\n", timebuf,
                        ts.tv_nsec / 1000000, level_str[level], event ? event : "", msgbuf);
    }
