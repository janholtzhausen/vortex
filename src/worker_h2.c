...
static void h2_stream_rst(struct h2_session *sess, struct h2_stream *st, uint32_t error_code);
...
static void h2_submit_unauthorized(struct h2_session *sess, struct h2_stream *st)
{
    ...
    nghttp2_nv nvs[] = {
        {.name = (uint8_t *)":status",
         ...
         .namelen = 7,
         .valuelen = sizeof(status) - 1,
         .flags = NGHTTP2_NV_FLAG_NO_COPY_NAME | NGHTTP2_NV_FLAG_NO_COPY_VALUE},
        {.name = (uint8_t *)"www-authenticate",
         .value = (uint8_t *)auth,
         .namelen = 16,
         .valuelen = sizeof(auth) - 1,
         .flags = NGHTTP2_NV_FLAG_NO_COPY_NAME | NGHTTP2_NV_FLAG_NO_COPY_VALUE},
...
static bool h2_auth_failed(struct h2_session *sess, struct h2_stream *st)
{
    ...
    if (ri < 0 || ri >= sess->w->cfg->route_count) return false;
...
static bool h2_try_cache_hit(struct h2_session *sess, struct h2_stream *st)
{
    ...
    if (!w->cache || !w->cache->index || strcmp(st->method, "GET") != 0) return false;

    char cache_key[2304];
    h2_make_cache_key(st, cache_key, sizeof(cache_key));
    struct cached_response cached;
    if (cache_fetch_copy(w->cache, cache_key, strlen(cache_key), &cached) != 0) return false;
    ...
    log_debug("h2_cache_hit", "cid=%u stream=%d url=%s", sess->cid, st->stream_id, cache_key);
    return true;
}
...
{
    struct worker *w = sess->w;
    if (!w->cache || !w->cache->index || strcmp(st->method, "GET") != 0) return;
    if (!st->resp_headers_done || !st->backend_eof || st->is_chunked_resp) return;
    ...
    if (st->resp_submitted && !st->prefer_buffered_resp) return;

    if (st->resp_buf_len < 12 || memcmp(st->resp_buf, "HTTP/", 5) != 0) return;
    ...
    uint32_t body_len =
        st->resp_buf_len > st->resp_hdr_end ? st->resp_buf_len - st->resp_hdr_end : 0u;
    if (body_len == 0) return;

    char cache_key[2304];
    h2_make_cache_key(st, cache_key, sizeof(cache_key));
    cache_store(w->cache, cache_key, strlen(cache_key), (uint16_t)status, ttl, st->resp_buf,
                st->resp_hdr_end, st->resp_buf + st->resp_hdr_end, body_len);
    log_debug("h2_cache_store", "cid=%u stream=%d url=%s ttl=%u body=%u", sess->cid, st->stream_id,
              cache_key, ttl, body_len);
}
...
static void h2_send_pending(struct h2_session *sess)
{
    ...
    if (sess->send_buf_off > 0) {
        if (sess->send_buf_off == sess->send_buf_len) {
            ...
            memmove(sess->send_buf, sess->send_buf + sess->send_buf_off,
    ...
    while (1) {
        ...
        if ((size_t)n > UINT32_MAX || !h2_send_buf_reserve(sess, (uint32_t)n)) {
            log_error("h2_send", "cid=%u failed to grow send buffer for %zd bytes", sess->cid, n);
    ...
    if (!sqe) return;
    io_uring_prep_send(sqe, h->client_fd, sess->send_buf + sess->send_buf_off, pending,
                       MSG_NOSIGNAL);
...
static void h2_stream_cleanup(struct h2_session *sess, struct h2_stream *st)
{
    ...
    if (st->grpc) {
        if (st->grpc->ngh2) {
            nghttp2_session_del(st->grpc->ngh2);
            st->grpc->ngh2 = NULL;
        }
...
{
    if (st->state != H2_STREAM_CLOSING) return;
    if (st->backend_send_in_flight || st->backend_recv_in_flight) return;
...
{
    if (st->stream_id > 0)
        nghttp2_submit_rst_stream(sess->ngh2, NGHTTP2_FLAG_NONE, st->stream_id, error_code);
    if (st->backend_fd >= 0) {
        close(st->backend_fd);
        st->backend_fd = -1;
    }
...
static void h2_arm_backend_recv(struct h2_session *sess, struct h2_stream *st)
{
    ...
    io_uring_prep_recv(sqe, st->backend_fd, st->resp_buf + st->resp_buf_len, space, 0);
...
static ssize_t h2_data_source_read(nghttp2_session *ngh2, int32_t stream_id, uint8_t *buf,
                                   ...
                                   void *user_data __attribute__((unused)))
{
    ...
    uint32_t body_total = st->resp_buf_len > body_start ? st->resp_buf_len - body_start : 0u;
...
static void h2_submit_response(struct h2_session *sess, struct h2_stream *st)
{
    ...
    if (ttl > 0) {
        if (ttl >= 3600)
            cc_val_len = snprintf(cc_val, sizeof(cc_val), "public, max-age=%u, immutable", ttl);
        else
            cc_val_len = snprintf(cc_val, sizeof(cc_val), "public, max-age=%u", ttl);
    }
                    ...
                    if (memcmp(tmp, "transfer-encoding", 17) == 0) {
                        ...
                        if (vlen >= 7 && memmem(v, vlen, "chunked", 7)) {
                            ...
                            uint32_t blen = st->resp_buf_len > st->resp_hdr_end
                                                ? st->resp_buf_len - st->resp_hdr_end
                                                : 0u;
    ...
    int nv_count = 1; /* 1 for :status */
    {
        ...
        while (q < hdr_end - 1) {
            const char *nl = (const char *)memmem(q, (size_t)(hdr_end - q), "\r\n", 2);
            if (!nl || nl == q) break;
            if (memchr(q, ':', (size_t)(nl - q))) nv_count++;
            q = nl + 2;
        }
    }
    ...
    while (p < hdr_end - 1 && idx < nv_count) {
        ...
        if (name_len == 6 && memcmp(name_p, "server", 6) == 0) {
            ...
            const char *srv_hdr =
                (ri >= 0 && ri < w->cfg->route_count && w->cfg->routes[ri].server_header[0])
        ...
        if (name_len == 4 && memcmp(name_p, "etag", 4) == 0) etag_seen = true;
    ...
    int rv = nghttp2_submit_response(sess->ngh2, st->stream_id, nvs, (size_t)idx,
    ...
    free(nvs);
    if (rv != 0) {
        log_error("h2_resp", "nghttp2_submit_response2 stream=%d: %s", st->stream_id,
                  nghttp2_strerror(rv));
        h2_stream_rst(sess, st, NGHTTP2_INTERNAL_ERROR);
        return;
    }
...
static int h2_build_http11_request(struct h2_stream *st, const char *backend_creds)
{
    ...
    if (auth_hdr[0]) p += snprintf(p, (size_t)(end - p), "%s", auth_hdr);
                                      ...
                                      struct h2_stream *st)
{
    ...
    if (bcfg->resolved_addrlen > 0) {
        memcpy(&st->backend_addr, &bcfg->resolved_addr, bcfg->resolved_addrlen);
        st->backend_addrlen = bcfg->resolved_addrlen;
    } else {
        ...
        memcpy(&st->backend_addr, res->ai_addr, res->ai_addrlen);
        st->backend_addrlen = (socklen_t)res->ai_addrlen;
        freeaddrinfo(res);
    }

    int fd = socket(st->backend_addr.ss_family, SOCK_STREAM | SOCK_NONBLOCK, 0);
    if (fd < 0) {
        h2_stream_rst(sess, st, NGHTTP2_INTERNAL_ERROR);
        return;
    }
    ...
    io_uring_prep_connect(sqe, fd, (struct sockaddr *)&st->backend_addr, st->backend_addrlen);
    ...
    log_debug("h2_connect", "cid=%u stream=%d slot=%u fd=%d -> %s", sess->cid, st->stream_id,
              (unsigned)st->slot, fd, bcfg->address);
}
...
static int on_begin_headers_cb(nghttp2_session *ngh2, const nghttp2_frame *frame, void *user_data)
{
    (void)ngh2;
    ...
    for (uint8_t i = 0; i < H2_STREAM_SLOTS; i++) {
        if (sess->streams[i].state == H2_STREAM_FREE) {
            ...
            st->cid = sess->cid;
            sess->active_streams++;
            log_debug("h2_stream", "cid=%u stream=%d slot=%u OPEN", sess->cid, st->stream_id,
                      (unsigned)i);
            return 0;
        }
    }
...
static int on_header_cb(nghttp2_session *ngh2, const nghttp2_frame *frame, const uint8_t *name,
                        ...
                        void *user_data)
{
    ...
    if (namelen == 12 && memcmp(name, "content-type", 12) == 0) {
        if (valuelen >= 16 && memcmp(value, "application/grpc", 16) == 0) st->is_grpc = true;
    }
...
static int on_frame_recv_cb(nghttp2_session *ngh2, const nghttp2_frame *frame, void *user_data)
{
    (void)ngh2;
    struct h2_session *sess = (struct h2_session *)user_data;
    ...
    if (frame->hd.type == NGHTTP2_HEADERS && (frame->hd.flags & NGHTTP2_FLAG_END_HEADERS) != 0) {
        if (end_stream) {
...
static int on_data_chunk_recv_cb(nghttp2_session *ngh2, uint8_t flags, int32_t stream_id,
                                 const uint8_t *data, size_t len, void *user_data)
{
    ...
    struct h2_stream *st = NULL;
    for (uint8_t i = 0; i < H2_STREAM_SLOTS; i++) {
        if (sess->streams[i].state != H2_STREAM_FREE && sess->streams[i].stream_id == stream_id) {
            st = &sess->streams[i];
            break;
        }
    }
...
static int on_stream_close_cb(nghttp2_session *ngh2, int32_t stream_id, uint32_t error_code,
                              void *user_data)
{
    (void)ngh2;
    (void)error_code;
    ...
    for (uint8_t i = 0; i < H2_STREAM_SLOTS; i++) {
        if (sess->streams[i].state != H2_STREAM_FREE && sess->streams[i].stream_id == stream_id) {
            log_debug("h2_stream", "cid=%u stream=%d CLOSED err=%u", sess->cid, stream_id,
                      error_code);
...
int h2_session_init(struct worker *w, uint32_t cid)
{
    ...
    nghttp2_submit_settings(sess->ngh2, NGHTTP2_FLAG_NONE, settings,
 ...
 */
bool h2_inject_predata(struct worker *w, uint32_t cid, const uint8_t *data, size_t len)
{
    struct conn_cold *cc = conn_cold_ptr(&w->pool, cid);
    ...
    if (r < 0) {
        log_error("h2_predata", "cid=%u nghttp2: %s", cid, nghttp2_strerror((int)r));
...
void h2_session_free(struct h2_session *sess)
{
    ...
    for (int i = 0; i < H2_STREAM_SLOTS; i++) {
        if (sess->streams[i].state != H2_STREAM_FREE) h2_stream_cleanup(sess, &sess->streams[i]);
    }
    if (sess->ngh2) {
        nghttp2_session_del(sess->ngh2);
        sess->ngh2 = NULL;
    }
    free(sess->send_buf);
    free(sess);
...
void h2_on_recv(struct worker *w, uint32_t cid, int n, const uint8_t *data, uint16_t buf_id,
                bool multishot_active)
{
    ...
    if (n <= 0) {
        ...
        if (w->uring.recv_ring && data && data >= w->uring.recv_ring_mem &&
            data < w->uring.recv_ring_mem + (size_t)w->uring.recv_ring_count * w->pool.buf_size)
            uring_recv_ring_return(&w->uring, buf_id, w->pool.buf_size);
        conn_close(w, cid, n < 0);
        return;
    }
    if (!sess) {
        if (w->uring.recv_ring && data) uring_recv_ring_return(&w->uring, buf_id, w->pool.buf_size);
        conn_close(w, cid, true);
        return;
    }
    ...
    if (ret < 0) {
        log_error("h2_recv", "cid=%u nghttp2_session_mem_recv: %s", cid,
    ...
    if (!multishot_active) {
        ...
        if (w->uring.recv_ring) {
            int _pfd = w->uring.files_registered ? FIXED_FD_CLIENT(w, cid) : h->client_fd;
            ...
            io_uring_prep_recv(sqe, h->client_fd, conn_recv_buf(&w->pool, cid), w->pool.buf_size,
                               0);
        }
...
void h2_on_send_client(struct worker *w, uint32_t cid, int sent)
{
    ...
    for (int i = 0; i < H2_STREAM_SLOTS; i++) {
        ...
        uint32_t pending = (st->resp_buf_len > st->resp_hdr_end + st->resp_body_sent)
                               ? st->resp_buf_len - st->resp_hdr_end - st->resp_body_sent
                               : 0u;
...
static void h2_grpc_send_pending(struct h2_session *sess, struct h2_stream *st)
{
    ...
    io_uring_prep_send(sqe, st->backend_fd, grpc->send_buf, grpc->send_len, MSG_NOSIGNAL);
...
static void h2_grpc_arm_backend_recv(struct h2_session *sess, struct h2_stream *st)
{
    ...
    io_uring_prep_recv(sqe, st->backend_fd, st->grpc->recv_buf, sizeof(st->grpc->recv_buf), 0);
...
static void h2_grpc_submit_response(struct h2_session *sess, struct h2_stream *st)
{
    ...
    char status_str[4];
    snprintf(status_str, sizeof(status_str), "%d", grpc->resp_status ? grpc->resp_status : 200);
    ...
    if (grpc->ntrailers > 0) {
        ...
        if (tvs) {
            ...
            nghttp2_submit_trailer(sess->ngh2, st->stream_id, tvs, (size_t)grpc->ntrailers);
            free(tvs);
        }
    }
...
static int grpc_be_on_header_cb(nghttp2_session *ngh2, const nghttp2_frame *frame,
...
static int grpc_be_on_data_chunk_cb(nghttp2_session *ngh2, uint8_t flags, int32_t stream_id,
                                    const uint8_t *data, size_t len, void *user_data)
{
    (void)ngh2;
    (void)flags;
    ...
    if (st->resp_buf_len + (uint32_t)len > H2_RESP_MAX) {
        log_warn("h2_grpc", "cid=%u stream=%d grpc response too large, RST", st->cid,
                 st->stream_id);
...
static int grpc_be_on_frame_recv_cb(nghttp2_session *ngh2, const nghttp2_frame *frame,
                                    void *user_data)
{
    (void)ngh2;
    struct h2_stream *st = (struct h2_stream *)user_data;
...
static ssize_t grpc_req_read_cb(nghttp2_session *ngh2, int32_t stream_id, uint8_t *buf,
                                size_t length, uint32_t *data_flags, nghttp2_data_source *source,
                                void *user_data __attribute__((unused)))
{
    (void)ngh2;
    ...
    if (grpc->req_body_sent >= st->req_body_len) *data_flags |= NGHTTP2_DATA_FLAG_EOF;
    return (ssize_t)to_copy;
}
...
static void h2_grpc_backend_init(struct h2_session *sess, struct h2_stream *st)
{
    ...
    nghttp2_submit_settings(grpc->ngh2, NGHTTP2_FLAG_NONE, settings,
                            sizeof(settings) / sizeof(settings[0]));
    ...
    nvs[idx++] = (nghttp2_nv){
        .name = (uint8_t *)":method",
        .namelen = 7,
        .value = (uint8_t *)st->method,
        .valuelen = strlen(st->method),
        .flags = NGHTTP2_NV_FLAG_NO_COPY_NAME | NGHTTP2_NV_FLAG_NO_COPY_VALUE,
    };
    nvs[idx++] = (nghttp2_nv){
        .name = (uint8_t *)":path",
        .namelen = 5,
        .value = (uint8_t *)st->path,
        .valuelen = strlen(st->path),
        .flags = NGHTTP2_NV_FLAG_NO_COPY_NAME | NGHTTP2_NV_FLAG_NO_COPY_VALUE,
    };
    nvs[idx++] = (nghttp2_nv){
        .name = (uint8_t *)":authority",
        .namelen = 10,
        .value = (uint8_t *)st->authority,
        .valuelen = strlen(st->authority),
        .flags = NGHTTP2_NV_FLAG_NO_COPY_NAME | NGHTTP2_NV_FLAG_NO_COPY_VALUE,
    };
    nvs[idx++] = (nghttp2_nv){
        .name = (uint8_t *)":scheme",
    ...
    log_debug("h2_grpc", "cid=%u stream=%d gRPC h2c backend init fd=%d", sess->cid, st->stream_id,
              st->backend_fd);
}
...
void h2_on_grpc_backend_recv(struct worker *w, uint32_t cid, uint32_t slot, int n)
{
    ...
    if (ret < 0) {
        log_error("h2_grpc", "cid=%u stream=%d nghttp2_session_mem_recv: %s", cid, st->stream_id,
                  nghttp2_strerror((int)ret));
        h2_stream_rst(sess, st, NGHTTP2_INTERNAL_ERROR);
        return;
    }
    ...
    if (!grpc->trailers_done) h2_grpc_arm_backend_recv(sess, st);
}
...
void h2_on_backend_connect(struct worker *w, uint32_t cid, uint32_t slot, int res)
{
    ...
    if (res < 0) {
        log_debug("h2_connect", "cid=%u stream=%d connect failed: %s", cid, st->stream_id,
                  strerror(-res));
        h2_stream_rst(sess, st, NGHTTP2_CONNECT_ERROR);
        return;
    }
    ...
    const char *bcreds =
        (ri >= 0 && ri < (int)w->cfg->route_count) ? w->cfg->routes[ri].backend_credentials : "";
    ...
    io_uring_prep_send(sqe, st->backend_fd, st->req_http11 + st->req_send_off,
...
void h2_on_backend_recv(struct worker *w, uint32_t cid, uint32_t slot, int n)
{
    ...
    if (st->state != H2_STREAM_WAITING_RESP && st->state != H2_STREAM_STREAMING) return;
    ...
    if (!st->resp_headers_done) {
        const uint8_t *hdr_end =
            (const uint8_t *)memmem(st->resp_buf, st->resp_buf_len, "\r\n\r\n", 4);
        if (hdr_end) {
            st->resp_headers_done = true;
            ...
            const uint8_t *te =
                (const uint8_t *)memmem(st->resp_buf, st->resp_hdr_end, "Transfer-Encoding:", 18);
            if (!te)
                te = (const uint8_t *)memmem(st->resp_buf, st->resp_hdr_end,
                                             "transfer-encoding:", 18);
            if (te) {
                const uint8_t *eol = (const uint8_t *)memmem(
                    te, (size_t)(st->resp_buf + st->resp_hdr_end - te), "\r\n", 2);
                if (eol && memmem(te, (size_t)(eol - te), "chunked", 7)) st->is_chunked_resp = true;
            }
             ...
             * complete. */
            const uint8_t *ct =
                (const uint8_t *)memmem(st->resp_buf, st->resp_hdr_end, "Content-Type:", 13);
            if (!ct)
                ct = (const uint8_t *)memmem(st->resp_buf, st->resp_hdr_end, "content-type:", 13);
            if (ct) {
                const uint8_t *eol = (const uint8_t *)memmem(
            ...
            if (!st->is_chunked_resp && !st->prefer_buffered_resp) h2_submit_response(sess, st);
        }
    }
    ...
    if (st->is_chunked_resp || st->prefer_buffered_resp) {
        ...
        if (st->resp_buf_len >= H2_RESP_MAX) {
            log_warn("h2_recv", "cid=%u stream=%d chunked response too large (>%u), RST", cid,
                     st->stream_id, H2_RESP_MAX);
            h2_stream_rst(sess, st, NGHTTP2_INTERNAL_ERROR);
            return;
        }
        h2_arm_backend_recv(sess, st);
    ...
    if (st->resp_submitted && st->resp_body_sent > 0) {
        ...
        if (remaining > 0)
            memmove(st->resp_buf + body_start, st->resp_buf + body_start + st->resp_body_sent,
                    remaining);
    ...
    if (pending < H2_STREAM_BUF_MAX) {
        ...
        log_debug("h2_stream", "cid=%u stream=%d recv paused (pending=%u)", cid, st->stream_id,
                  pending);
    }
}
