 ...
 */
static const uint8_t *memmem_avx2(const uint8_t *hay, size_t hlen, const uint8_t *needle,
                                  size_t nlen)
{
    ...
    for (; p + 32 <= end; p += 32) {
        ...
        while (mask) {
            ...
            if (memcmp(p + i, needle, nlen) == 0) return p + i;
...
static const uint8_t *find_crlf_avx2(const uint8_t *p, size_t len)
{
    ...
    for (; p + 32 <= end; p += 32) {
        ...
        uint32_t mask = (uint32_t)_mm256_movemask_epi8(
            _mm256_and_si256(_mm256_cmpeq_epi8(a, cr), _mm256_cmpeq_epi8(b, lf)));
...
static const uint8_t *find_hdr_end_avx2(const uint8_t *p, size_t len)
{
    ...
    for (; p + 32 <= end; p += 32) {
        __m256i block = _mm256_loadu_si256((const __m256i *)p);
        uint32_t mask = (uint32_t)_mm256_movemask_epi8(_mm256_cmpeq_epi8(block, cr));
        while (mask) {
            int i = __builtin_ctz(mask);
