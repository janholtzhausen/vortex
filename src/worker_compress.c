...
size_t brotli_compress(const uint8_t *src, size_t src_len, uint8_t *dst, size_t dst_max)
{
    ...
    if (!BrotliEncoderCompress(6, BROTLI_DEFAULT_WINDOW, BROTLI_MODE_TEXT, src_len, src, &out_len,
                               dst))
        return 0;
    return out_len;
}
...
size_t compress_http_response_parts(uint8_t *headers, size_t header_len, const uint8_t *body,
                                    size_t body_len, uint8_t *scratch, size_t buf_size,
                                    bool prefer_br, bool *used_brotli, size_t *compressed_len_out)
{
    if (!headers || !body || !scratch || header_len < 4 || body_len < COMPRESS_MIN_BODY) return 0;

    size_t clen = prefer_br ? brotli_compress(body, body_len, scratch, buf_size)
    ...
    if (clen == 0 || clen >= body_len) return 0;
    ...
    uint8_t *vs = clh + 17;
    while (vs < headers + hdr_end_off && (*vs == ' ' || *vs == '\t'))
        vs++;
    ...
    if (hdr_end_off + 4 + delta >= buf_size) return 0;
    ...
    if (new_body_off + clen > buf_size) return 0;
    ...
    if (used_brotli) *used_brotli = prefer_br;
    if (compressed_len_out) *compressed_len_out = clen;
