...
static long cfg_strtol(const char *s)
{
    char *e;
    long v = strtol(s, &e, 10);
    return (e && e != s) ? v : 0;
}
static unsigned long cfg_strtoul(const char *s)
{
    char *e;
...
static int validate_no_crlf(const char *val, const char *field_name, int route_idx)
{
    for (const char *p = val; *p; p++) {
        if (*p == '\r' || *p == '\n') {
            log_error("config", "route %d %s contains CR/LF - refusing to start", route_idx,
                      field_name);
            return -1;
        }
    }
...
void config_set_defaults(struct vortex_config *cfg)
{
    ...
    cfg->ipv4_only =
        true; /* dual-stack off by default; set false to bind AF_INET6 with v4-mapped support */
    ...
    snprintf(cfg->server_header, sizeof(cfg->server_header), "%s",
             "CSWS/2.4.62 OpenVMS/V9.2-2 (Alpha)");
...
{
    if (tls_out) *tls_out = false;
    if (!addr) return "";
    if (strncmp(addr, "https://", 8) == 0) {
        if (tls_out) *tls_out = true;
        return addr + 8;
    }
    if (strncmp(addr, "http://", 7) == 0) return addr + 7;
    return addr;
}
...
static void handle_scalar(parser_ctx_t *ctx, const char *val_raw)
{
    ...
    case P_GLOBAL:
        if (!strcmp(k, "workers"))
            c->workers = (int)cfg_strtol(val);
        else if (!strcmp(k, "compress_pool_threads"))
            c->compress_pool_threads = (int)cfg_strtol(val);
        else if (!strcmp(k, "sqpoll"))
            c->sqpoll = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "hugepages"))
            c->hugepages = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "cpu_affinity"))
            c->cpu_affinity = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "ipv4_only"))
            c->ipv4_only = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "bind_address"))
            snprintf(c->bind_address, sizeof(c->bind_address), "%s", val);
        else if (!strcmp(k, "bind_port"))
            c->bind_port = (uint16_t)cfg_strtol(val);
        else if (!strcmp(k, "http_port"))
            c->http_port = (uint16_t)cfg_strtol(val);
        else if (!strcmp(k, "interface"))
            snprintf(c->interface, sizeof(c->interface), "%s", val);
        else if (!strcmp(k, "log_level"))
            snprintf(c->log_level, sizeof(c->log_level), "%s", val);
        else if (!strcmp(k, "log_format"))
            snprintf(c->log_format, sizeof(c->log_format), "%s", val);
        else if (!strcmp(k, "pid_file"))
            snprintf(c->pid_file, sizeof(c->pid_file), "%s", val);
        else if (!strcmp(k, "server_header"))
            snprintf(c->server_header, sizeof(c->server_header), "%s",
                     !strcmp(val, "none") ? "" : val);
        else if (!strcmp(k, "congestion_control"))
            snprintf(c->congestion_control, sizeof(c->congestion_control), "%s", val);
        else if (!strcmp(k, "max_request_body_mb"))
            ...
            snprintf(c->run_as_user, sizeof(c->run_as_user), "%s", val);
        break;

    case P_TLS:
        if (!strcmp(k, "min_version"))
            c->tls.min_version = !strcmp(val, "1.2") ? 0x0303 : 0x0304;
        else if (!strcmp(k, "max_version"))
            c->tls.max_version = !strcmp(val, "1.3") ? 0x0304 : 0x0303;
        else if (!strcmp(k, "ciphersuites"))
            snprintf(c->tls.ciphersuites, sizeof(c->tls.ciphersuites), "%s", val);
        else if (!strcmp(k, "session_timeout"))
            c->tls.session_timeout = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "session_ticket_rotation"))
            c->tls.session_ticket_rotation = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "ktls"))
            c->tls.ktls = !strcmp(val, "true") || !strcmp(val, "yes");
        break;

    case P_XDP:
        if (!strcmp(k, "mode")) {
            if (!strcmp(val, "native"))
                c->xdp.mode = XDP_MODE_NATIVE;
            else if (!strcmp(val, "skb"))
                c->xdp.mode = XDP_MODE_SKB;
            else
                c->xdp.mode = XDP_MODE_AUTO;
        } else if (!strcmp(k, "blocklist_file"))
            snprintf(c->xdp.blocklist_file, sizeof(c->xdp.blocklist_file), "%s", val);
        break;

    case P_XDP_RATELIMIT:
        if (!strcmp(k, "enabled"))
            c->xdp.rate_limit_enabled = !strcmp(val, "true");
        else if (!strcmp(k, "requests_per_second"))
            c->xdp.rate_limit_rps = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "burst"))
            c->xdp.rate_limit_burst = (uint32_t)cfg_strtoul(val);
        break;

    case P_XDP_PROTECTED_PORTS:
        /* Each scalar is a port number */
        if (c->xdp.protected_ports_count < 16) {
            uint16_t port = (uint16_t)cfg_strtol(val);
    ...
    case P_CACHE:
        if (!strcmp(k, "enabled"))
            c->cache.enabled = !strcmp(val, "true");
        else if (!strcmp(k, "etag_sha256"))
            c->cache.etag_sha256 = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "verify_crc"))
            c->cache.verify_crc = !strcmp(val, "true") || !strcmp(val, "yes");
        else if (!strcmp(k, "index_entries"))
            c->cache.index_entries = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "slab_size_mb"))
            c->cache.slab_size_bytes = (uint64_t)cfg_strtoul(val) * 1024 * 1024;
        else if (!strcmp(k, "default_ttl"))
            c->cache.default_ttl = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "use_hugepages"))
            c->cache.use_hugepages = !strcmp(val, "true");
        else if (!strcmp(k, "disk_cache_path"))
            snprintf(c->cache.disk_cache_path, sizeof(c->cache.disk_cache_path), "%s", val);
        else if (!strcmp(k, "disk_slab_size_mb"))
            c->cache.disk_slab_size_bytes = (uint64_t)cfg_strtoul(val) * 1024 * 1024;
        break;

    case P_ACME:
        if (!strcmp(k, "enabled"))
            c->acme.enabled = !strcmp(val, "true");
        else if (!strcmp(k, "email"))
            snprintf(c->acme.email, sizeof(c->acme.email), "%s", val);
        else if (!strcmp(k, "directory_url"))
            snprintf(c->acme.directory_url, sizeof(c->acme.directory_url), "%s", val);
        else if (!strcmp(k, "account_key_path"))
            snprintf(c->acme.account_key_path, sizeof(c->acme.account_key_path), "%s", val);
        else if (!strcmp(k, "storage_path"))
            snprintf(c->acme.storage_path, sizeof(c->acme.storage_path), "%s", val);
        else if (!strcmp(k, "renewal_days_before_expiry"))
            c->acme.renewal_days = (int)cfg_strtol(val);
        else if (!strcmp(k, "preferred_challenge"))
            snprintf(c->acme.preferred_challenge, sizeof(c->acme.preferred_challenge), "%s", val);
        else if (!strcmp(k, "dns_provider"))
            snprintf(c->acme.dns_provider, sizeof(c->acme.dns_provider), "%s", val);
        break;

    case P_ACME_DNS_CFG:
        if (!strcmp(k, "api_token"))
            snprintf(c->acme.dns_api_token, sizeof(c->acme.dns_api_token), "%s", val);
        break;

    case P_METRICS:
        if (!strcmp(k, "enabled"))
            c->metrics.enabled = !strcmp(val, "true");
        else if (!strcmp(k, "bind_address"))
            snprintf(c->metrics.bind_address, sizeof(c->metrics.bind_address), "%s", val);
        else if (!strcmp(k, "port"))
            c->metrics.port = (uint16_t)cfg_strtol(val);
        else if (!strcmp(k, "path"))
            snprintf(c->metrics.path, sizeof(c->metrics.path), "%s", val);
        break;

    case P_DASHBOARD:
        if (!strcmp(k, "enabled"))
            c->dashboard.enabled = !strcmp(val, "true");
        else if (!strcmp(k, "bind_address"))
            snprintf(c->dashboard.bind_address, sizeof(c->dashboard.bind_address), "%s", val);
        else if (!strcmp(k, "port"))
            c->dashboard.port = (uint16_t)cfg_strtol(val);
        break;

    case P_ROUTE: {
        struct route_config *r = &c->routes[ctx->route_idx];
        if (!strcmp(k, "backend_timeout_ms"))
            r->backend_timeout_ms = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "x_api_key"))
            snprintf(r->x_api_key, sizeof(r->x_api_key), "%s", val);
        else if (!strcmp(k, "backend_credentials"))
            snprintf(r->backend_credentials, sizeof(r->backend_credentials), "%s", val);
        else if (!strcmp(k, "server_header"))
            snprintf(r->server_header, sizeof(r->server_header), "%s",
                     !strcmp(val, "none") ? "" : val);
        else if (!strcmp(k, "congestion_control"))
            snprintf(r->congestion_control, sizeof(r->congestion_control), "%s", val);
        else if (!strcmp(k, "backend_auth")) {
            if (!strcmp(val, "passthrough"))
                r->backend_auth_mode = BACKEND_AUTH_PASSTHROUGH;
            else if (!strcmp(val, "rewrite"))
                r->backend_auth_mode = BACKEND_AUTH_REWRITE;
            else
                r->backend_auth_mode = BACKEND_AUTH_BLOCK;
        } else if (!strcmp(k, "hostname"))
            snprintf(r->hostname, sizeof(r->hostname), "%s", val);
        else if (!strcmp(k, "protocol")) {
            if (!strcmp(val, "tcp"))
                r->route_type = ROUTE_TYPE_TCP_TUNNEL;
            else
                r->route_type = ROUTE_TYPE_HTTP;
        } else if (!strcmp(k, "load_balancing")) {
            if (!strcmp(val, "weighted_round_robin"))
                r->lb_algo = LB_WEIGHTED_ROUND_ROBIN;
            else if (!strcmp(val, "least_conn"))
                r->lb_algo = LB_LEAST_CONN;
            else if (!strcmp(val, "ip_hash"))
                r->lb_algo = LB_IP_HASH;
            else
                r->lb_algo = LB_ROUND_ROBIN;
        } else if (!strcmp(k, "cert_provider")) {
            if (!strcmp(val, "acme_http01"))
                r->cert_provider = CERT_PROVIDER_ACME_HTTP01;
            else if (!strcmp(val, "acme_dns01"))
                r->cert_provider = CERT_PROVIDER_ACME_DNS01;
            else
                r->cert_provider = CERT_PROVIDER_STATIC;
        } else if (!strcmp(k, "cert_path"))
            snprintf(r->cert_path, sizeof(r->cert_path), "%s", val);
        else if (!strcmp(k, "key_path"))
            snprintf(r->key_path, sizeof(r->key_path), "%s", val);
        break;
    }
    ...
    case P_ROUTE_BACKEND: {
        ...
        if (!strcmp(k, "address")) {
            ...
            if (addr_tls) b->tls = true;
        } else if (!strcmp(k, "weight")) {
            b->weight = (uint16_t)cfg_strtol(val);
        } else if (!strcmp(k, "pool_size")) {
            b->pool_size = (int)cfg_strtol(val);
    ...
    case P_ROUTE_CACHE: {
        ...
        if (!strcmp(k, "enabled"))
            rc->enabled = !strcmp(val, "true");
        else if (!strcmp(k, "ttl"))
            rc->ttl = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "key"))
            snprintf(rc->key_pattern, sizeof(rc->key_pattern), "%s", val);
        break;
    }
    ...
    case P_ROUTE_AUTH: {
        ...
        if (!strcmp(k, "enabled"))
            a->enabled = !strcmp(val, "true");
        else if (!strcmp(k, "file"))
            snprintf(a->file, sizeof(a->file), "%s", val);
        break;
    }
    case P_ROUTE_AUTH_USERS: {
        ...
        if (a->credential_count < VORTEX_MAX_AUTH_USERS) {
            if (!auth_parse_verifier(&a->verifiers[a->credential_count], val)) {
                log_error("config_load", "route=%s invalid auth verifier for user entry %d",
                          c->routes[ctx->route_idx].hostname[0] ? c->routes[ctx->route_idx].hostname
                                                                : "<unknown>",
                          a->credential_count);
                ctx->error = -1;
                break;
    ...
    case P_ROUTE_RATELIMIT: {
        ...
        if (!strcmp(k, "enabled"))
            rl->enabled = !strcmp(val, "true");
        else if (!strcmp(k, "rps"))
            rl->rps = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "burst"))
            rl->burst = (uint32_t)cfg_strtoul(val);
        break;
    }

    case P_ROUTE_HEALTH: {
        struct route_health_config *hc = &c->routes[ctx->route_idx].health;
        if (!strcmp(k, "fail_threshold"))
            hc->fail_threshold = (uint32_t)cfg_strtoul(val);
        else if (!strcmp(k, "open_ms"))
            hc->open_ms = (uint32_t)cfg_strtoul(val);
        break;
    }
    ...
    case P_ROUTE_BACKEND_HEADER: {
        ...
        if (ctx->backend_header_idx < 0 ||
            ctx->backend_header_idx >= VORTEX_MAX_BACKEND_HEADER_RULES)
            break;
        ...
        if (!strcmp(k, "name")) {
            ...
            if (!strcmp(val, "set"))
                hr->action = HEADER_ACTION_SET;
            else
                hr->action = HEADER_ACTION_BLOCK;
    ...
    case P_ROUTE_RESPONSE_HEADER: {
        ...
        if (ctx->response_header_idx < 0 ||
            ctx->response_header_idx >= VORTEX_MAX_RESPONSE_HEADER_RULES)
            break;
...
int config_load(const char *path, struct vortex_config *cfg)
{
    ...
    while (1) {
        if (!yaml_parser_parse(&parser, &ev)) {
            log_error("config_load", "YAML parse error at line %zu: %s", parser.problem_mark.line,
                      parser.problem);
            ret = -1;
            break;
        }
        ...
        case YAML_MAPPING_END_EVENT:
            ...
            if (ctx.state == P_ROUTE_BACKEND) {
                ctx.state = P_ROUTE_BACKENDS;
            } else if (ctx.state == P_ROUTE_BACKENDS) {
                ctx.state = P_ROUTE;
            } else if (ctx.state == P_ROUTE_CACHE) {
                ctx.state = P_ROUTE;
            } else if (ctx.state == P_ROUTE_AUTH) {
                ctx.state = P_ROUTE;
            } else if (ctx.state == P_ROUTE_RATELIMIT) {
                ctx.state = P_ROUTE;
            } else if (ctx.state == P_ROUTE_HEALTH) {
                ctx.state = P_ROUTE;
            } else if (ctx.state == P_ROUTE_BACKEND_HEADER) {
                ctx.state = P_ROUTE_BACKEND_HEADERS;
            } else if (ctx.state == P_ROUTE_RESPONSE_HEADER) {
                ctx.state = P_ROUTE_RESPONSE_HEADERS;
            } else if (ctx.state == P_ROUTE) {
                ctx.state = P_ROUTES;
            } else if (ctx.state == P_ACME_DNS_CFG) {
                ctx.state = P_ACME;
            } else if (ctx.state == P_XDP_RATELIMIT) {
                ctx.state = P_XDP;
            }
            /* Section mappings pop back to root */
            else if (ctx.state == P_GLOBAL || ctx.state == P_TLS || ctx.state == P_XDP ||
                     ctx.state == P_CACHE || ctx.state == P_ACME || ctx.state == P_METRICS ||
        ...
        case YAML_SCALAR_EVENT: {
            ...
            if (!got_key) {
                ...
                if (ctx.state == P_ROOT) {
                    if (!strcmp(sv, "global"))
                        ctx.state = P_GLOBAL;
                    else if (!strcmp(sv, "tls"))
                        ctx.state = P_TLS;
                    else if (!strcmp(sv, "xdp"))
                        ctx.state = P_XDP;
                    else if (!strcmp(sv, "cache"))
                        ctx.state = P_CACHE;
                    else if (!strcmp(sv, "acme"))
                        ctx.state = P_ACME;
                    else if (!strcmp(sv, "metrics"))
                        ctx.state = P_METRICS;
                    else if (!strcmp(sv, "dashboard"))
                        ctx.state = P_DASHBOARD;
                    ...
                    if (!strcmp(sv, "backends"))
                        ctx.state = P_ROUTE_BACKENDS;
                    else if (!strcmp(sv, "cache"))
                        ctx.state = P_ROUTE_CACHE;
                    else if (!strcmp(sv, "auth"))
                        ctx.state = P_ROUTE_AUTH;
                    else if (!strcmp(sv, "rate_limit"))
                        ctx.state = P_ROUTE_RATELIMIT;
                    else if (!strcmp(sv, "health_check"))
                        ctx.state = P_ROUTE_HEALTH;
                    else if (!strcmp(sv, "backend_headers"))
                        ctx.state = P_ROUTE_BACKEND_HEADERS;
                }
            } else {
                handle_scalar(&ctx, sv);
    ...
    if (ret == 0 && ctx.error == 0) {
        for (int i = 0; i < cfg->route_count; i++) {
            ...
            if (route->auth.file[0]) {
                if (!auth_load_verifiers_file(&route->auth, route->auth.file, route->hostname)) {
                    log_error("config_load", "route=%s failed to load auth file %s",
                              route->hostname[0] ? route->hostname : "<unknown>", route->auth.file);
                    ret = -1;
                    break;
                }
            }
            if (route->auth.enabled && route->auth.credential_count == 0) {
                log_error("config_load", "route=%s auth enabled but no verifiers configured",
...
struct vortex_config *config_alloc(void)
{
    struct vortex_config *cfg = calloc(1, sizeof(*cfg));
    if (cfg) config_set_defaults(cfg);
    return cfg;
}

/* Global live config pointer — atomically swapped on SIGHUP reload */
_Atomic(struct vortex_config *) g_live_cfg = NULL;

/* config_reload: load new values into the pre-allocated heap config `new_cfg`.
 * Validates topology against the current live config (g_live_cfg).
 * Caller does the atomic pointer swap and grace-period free. */
int config_reload(const char *path, struct vortex_config *new_cfg)
{
    if (config_load(path, new_cfg) != 0) return -1;
    const struct vortex_config *cur = config_live();
    if (!cur) return 0; /* first load — no validation needed */
    if (new_cfg->route_count != cur->route_count) {
        log_warn("config_reload",
                 "refusing reload: route_count changed from %d to %d; restart required",
                 cur->route_count, new_cfg->route_count);
        return -1;
    }
    for (int ri = 0; ri < cur->route_count; ri++) {
        const struct route_config *old_route = &cur->routes[ri];
        const struct route_config *nr = &new_cfg->routes[ri];
        if (strcmp(old_route->hostname, nr->hostname) != 0) {
            log_warn("config_reload",
                     "refusing reload: route %d hostname changed (%s -> %s); restart required", ri,
                     old_route->hostname, nr->hostname);
            return -1;
        }
        if (old_route->backend_count != nr->backend_count) {
            log_warn("config_reload",
                     "refusing reload: route %d backend_count changed (%u -> %u); restart required",
                     ri, old_route->backend_count, nr->backend_count);
            return -1;
        }
        for (int bi = 0; bi < old_route->backend_count; bi++) {
            const struct backend_config *ob = &old_route->backends[bi];
            const struct backend_config *nb = &nr->backends[bi];
            if (strcmp(ob->address, nb->address) != 0 || ob->tls != nb->tls) {
                log_warn("config_reload",
                         "refusing reload: route %d backend %d changed (%s/%d -> %s/%d); restart "
                         "required",
                         ri, bi, ob->address, ob->tls, nb->address, nb->tls);
                return -1;
            }
        }
    }
...
void config_resolve_backends(struct vortex_config *cfg)
{
    for (int ri = 0; ri < cfg->route_count; ri++) {
        ...
        for (int bi = 0; bi < route->backend_count; bi++) {
            ...
            if (b->tls && !b->verify_peer_set) b->verify_peer = true;
            ...
            if (b->resolved_addrlen > 0) {
                log_info("config_resolve", "route=%s backend=%s: resolved ok", route->hostname,
                         addr_str);
            } else {
