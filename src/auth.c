...
static int b64_decode(const char *in, size_t in_len, char *out, size_t out_max)
{
    ...
    for (size_t i = 0; i + 3 < in_len; i += 4) {
        ...
        if (in[i + 2] != '=' && out_pos < out_max - 1)
            out[out_pos++] = (char)((v1 << 4) | (v2 >> 2));
...
static bool auth_parse_u32(const char *s, uint32_t *out)
{
    ...
    if (!s[0] || !end || *end != '\0' || v > UINT32_MAX) return false;
    *out = (uint32_t)v;
    return true;
}
...
static void pbkdf2_sha256(const uint8_t *pass, size_t pass_len, const uint8_t *salt,
                          size_t salt_len, uint32_t c, uint8_t *dk, size_t dk_len)
{
...
static int romix(uint32_t *b, uint64_t n, uint32_t r)
{
    ...
    uint32_t *x = malloc((size_t)256 * r + 64);
    if (!v || !x) {
        free(v);
        free(x);
        return -1;
    ...
    for (uint64_t i = 0; i < n; i++) {
        uint64_t j =
            ((uint64_t)x[(2 * r - 1) * 16] | ((uint64_t)x[(2 * r - 1) * 16 + 1] << 32)) & (n - 1);
 ...
 */
static int vortex_scrypt(const uint8_t *pass, size_t pass_len, const uint8_t *salt, size_t salt_len,
                         uint64_t N, uint32_t r, uint32_t p, uint8_t *dk, size_t dk_len)
{
    size_t mflen = 128 * r;
...
static bool auth_verify_password(const struct auth_verifier *verifier, const char *password)
{
    uint8_t derived[VORTEX_AUTH_MAX_HASH_LEN];
    if (verifier->hash_len == 0 || verifier->hash_len > sizeof(derived)) return false;
#ifdef VORTEX_PHASE_TLS
    if (vortex_scrypt((const uint8_t *)password, strlen(password), verifier->salt,
                      verifier->salt_len, 1ULL << verifier->log_n, verifier->r, verifier->p,
...
static bool auth_username_match_ct(const char *a, size_t la, const char *b)
{
    size_t lb = strlen(b);
...
static void auth_make_dummy_verifier(struct auth_verifier *dst, const struct auth_verifier *src)
{
    *dst = *src;
    if (dst->hash_len > 0) dst->hash[0] ^= 0xFF;
}
#endif
...
bool auth_parse_verifier(struct auth_verifier *out, const char *value)
{
    ...
    char *outer_ctx = NULL;
    char *username = strtok_r(buf, ":", &outer_ctx);
    char *inner_ctx = NULL;
    char *scheme = strtok_r(outer_ctx, "$", &inner_ctx);
    char *params = strtok_r(NULL, "$", &inner_ctx);
    char *salt_b64 = strtok_r(NULL, "$", &inner_ctx);
    char *hash_b64 = strtok_r(NULL, "$", &inner_ctx);
    char *extra = strtok_r(NULL, "$", &inner_ctx);

    if (!username || !scheme || !params || !salt_b64 || !hash_b64 || extra) return false;
    if (username[0] == '\0' || strlen(username) >= sizeof(out->username)) return false;
    if (strcmp(scheme, "scrypt") != 0) return false;

    uint32_t log_n = 0, r = 0, p = 0;
    char *param_ctx = NULL;
    for (char *tok = strtok_r(params, ",", &param_ctx); tok;
        ...
        *eq = '\0';
        ...
        const char *val = eq + 1;
        if (strcmp(key, "ln") == 0) {
            if (!auth_parse_u32(val, &log_n)) return false;
        } else if (strcmp(key, "r") == 0) {
            if (!auth_parse_u32(val, &r)) return false;
        } else if (strcmp(key, "p") == 0) {
            if (!auth_parse_u32(val, &p)) return false;
        } else {
            return false;
        }
    }

    if (log_n < 1 || log_n > 20 || r == 0 || p == 0) return false;

    char salt_decoded[(VORTEX_AUTH_MAX_SALT_LEN * 4 / 3) + 8];
    char hash_decoded[(VORTEX_AUTH_MAX_HASH_LEN * 4 / 3) + 8];
    int salt_len = b64_decode(salt_b64, strlen(salt_b64), salt_decoded, sizeof(salt_decoded));
    int hash_len = b64_decode(hash_b64, strlen(hash_b64), hash_decoded, sizeof(hash_decoded));
    if (salt_len <= 0 || salt_len > VORTEX_AUTH_MAX_SALT_LEN) return false;
    if (hash_len <= 0 || hash_len > VORTEX_AUTH_MAX_HASH_LEN) return false;
...
bool auth_load_verifiers_file(struct route_auth_config *auth, const char *path,
                              const char *route_hostname)
{
    if (!auth || !path || !path[0]) return false;
...
bool auth_check_basic_value(const struct route_auth_config *auth, const char *value,
                            size_t value_len)
{
    ...
    /* Decode base64 */
    char decoded[384] = {0}; /* max decoded "user:pass" */
    int decoded_len = b64_decode(v, tok_len, decoded, sizeof(decoded));
    if (decoded_len < 0) return false;

    char *sep = memchr(decoded, ':', (size_t)decoded_len);
    if (!sep || sep == decoded) return false;
    *sep = '\0';
    const char *username = decoded;
    const char *password = sep + 1;
    size_t username_len = (size_t)(sep - decoded);
    ...
    for (int i = 0; i < auth->credential_count; i++) {
        ...
        if (auth_username_match_ct(username, username_len, verifier->username)) selected = verifier;
    }
...
bool auth_check_request(const struct route_auth_config *auth, const uint8_t *req, int req_len)
{
    if (!auth || !auth->enabled) return true; /* auth not configured */
    if (auth->credential_count == 0) return false;
