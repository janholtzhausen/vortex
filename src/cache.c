               ...
               const char *disk_path, size_t disk_size, bool etag_sha256, bool verify_crc)
{
    ...
    log_info("cache_init", "index_entries=%zu index_bytes=%zu ram_slab=%zu", (size_t)index_entries,
             index_bytes, slab_size);
    ...
    if (disk_path && disk_path[0]) {
        if (disk_size == 0) {
            ...
            if (disk_size < 128ULL * 1024 * 1024) disk_size = 128ULL * 1024 * 1024;
        ...
        if (fd >= 0) {
            if (ftruncate(fd, (off_t)disk_size) == 0) {
                void *dm = mmap(NULL, disk_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
                if (dm != MAP_FAILED) {
                    c->disk_slab = dm;
                    c->disk_slab_size = disk_size;
                    log_info("cache_init", "disk_slab path=%s size=%zu", disk_path, disk_size);
                } else {
...
static struct cache_index_entry *cache_lookup_locked(struct cache *c, const char *url,
                                                     size_t url_len)
{
                ...
                const uint8_t *headers, size_t header_len, const uint8_t *body, size_t body_len)
{
    ...
    if (use_disk) {
        ...
        if (headers && header_len > 0) memcpy(c->slab + slab_off, headers, header_len);
        if (body && body_len > 0) memcpy(c->slab + slab_off + header_len, body, body_len);
        c->slab_watermark += total;
    }

    const uint8_t *stored =
        use_disk ? c->disk_slab + (slab_off & ~CACHE_SLAB_DISK_FLAG) : c->slab + slab_off;
...
int cache_fetch_copy(struct cache *c, const char *url, size_t url_len, struct cached_response *out)
{
    ...
    if (!e || !cache_entry_valid(e)) return -1;
    ...
    if (c->verify_crc) {
        ...
        if (actual != e->crc32c) {
            log_warn("cache_crc", "crc mismatch url=%.*s expected=%08x actual=%08x", (int)url_len,
                     url, e->crc32c, actual);
            e->flags = 0;
            c->evictions++;
            c->misses++;
...
uint32_t cache_ttl_for_url(const char *url)
{
    ...
    if (strncmp(url, "/api/", 5) == 0 || strncmp(url, "/api?", 5) == 0) return 0;
    ...
    if (ext) {
        ...
        for (const char *c = ext; *c && *c != '?' && *c != '#' && elen < 15; c++)
            e[elen++] = *c;
        e[elen] = '\0';
        if (!strcmp(e, ".js") || !strcmp(e, ".css") || !strcmp(e, ".woff") ||
            !strcmp(e, ".woff2") || !strcmp(e, ".ttf") || !strcmp(e, ".eot") ||
            !strcmp(e, ".png") || !strcmp(e, ".jpg") || !strcmp(e, ".jpeg") || !strcmp(e, ".svg") ||
            !strcmp(e, ".ico") || !strcmp(e, ".gif") || !strcmp(e, ".webp") || !strcmp(e, ".map")) {
            return 3600;
        }
    }
