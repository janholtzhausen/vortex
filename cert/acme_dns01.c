...
static int dns01_json_get_str(const char *json, const char *key, char *out, size_t out_max)
{
    char needle[256];
...
static int dns01_json_get_array_first(const char *json, const char *key, char *out, size_t out_max)
{
    char needle[256];
    snprintf(needle, sizeof(needle), "\"%s\"", key);
    const char *p = strstr(json, needle);
 ...
 * "token" and "url".
 */
static int dns01_find_dns01_challenge(const char *json, char *token, size_t token_max, char *url,
                                      size_t url_max)
{
    const char *p = json;
...
static int dns01_get_nonce(struct acme_client *cl, char *nonce, size_t nmax)
{
    int status = 0;
    char loc[8] = "";
...
static int dns01_post(struct acme_client *cl, const char *url,
                      const char *payload_json, /* NULL for POST-as-GET */
                      char *nonce, /* in: current nonce; out: fresh nonce */
                      size_t nonce_max, char *loc_out, size_t loc_max, int *status_out,
                      char **body_out)
{
    char *jws = make_jws(cl->account_key_priv, cl->account_key_pub, nonce, url, payload_json,
                         cl->account_url, cl->jwk_thumbprint);
    if (!jws) return -1;

    int r = https_request(cl, "POST", url, "application/jose+json", jws, strlen(jws), status_out,
                          loc_out, loc_max, nonce, nonce_max, body_out, NULL);
    free(jws);
    return r;
}
...
static int dns01_poll_for_status(struct acme_client *cl, const char *url, const char *want_status,
                                 int timeout_s)
{
    ...
    for (;;) {
        ...
        if (dns01_post(cl, url, NULL /* POST-as-GET */, nonce, sizeof(nonce), loc, sizeof(loc),
                       &status, &body) < 0) {
            free(body);
            return -1;
        }
        ...
        char obj_status[64] = "";
        dns01_json_get_str(body ? body : "", "status", obj_status, sizeof(obj_status));
        free(body);

        log_debug("acme_dns01", "poll %s → status=%s (want %s)", url, obj_status, want_status);
        ...
        if (time(NULL) >= deadline) {
            log_error("acme_dns01", "timeout polling %s (last status=%s)", url, obj_status);
            return -1;
        }
        sleep(3);
    }
...
int acme_obtain_dns01(struct acme_dns01_ctx *dctx, const char *domain, struct cert_result *out)
{
    ...
    if (dns01_get_nonce(cl, nonce, sizeof(nonce)) < 0) goto done;
    {
        ...
        snprintf(pay, sizeof(pay), "{\"identifiers\":[{\"type\":\"dns\",\"value\":\"%s\"}]}",
                 domain);
        if (dns01_post(cl, cl->newOrder_url, pay, nonce, sizeof(nonce), loc, sizeof(loc), &status,
                       &resp) < 0 ||
            (status != 201 && status != 200)) {
            log_error("acme_dns01", "newOrder returned %d: %s", status, resp ? resp : "");
            goto done;
        }
    }
    ...
    {
        char loc2[8] = "";
        if (dns01_post(cl, authz_url, NULL /* POST-as-GET */, nonce, sizeof(nonce), loc2,
                       sizeof(loc2), &status, &resp) < 0 ||
            status != 200) {
    ...
    if (dns01_find_dns01_challenge(resp ? resp : "", challenge_token, sizeof(challenge_token),
    ...
    char key_auth[512];
    {
        int n = snprintf(key_auth, sizeof(key_auth), "%s.%s", challenge_token, cl->jwk_thumbprint);
    ...
    if (dctx->dns_ops->create_txt(dctx->dns_ctx, txt_name, txt_value, txt_record_id,
                                  sizeof(txt_record_id)) < 0) {
    ...
    {
        int wait_s = dctx->propagation_wait_s > 0 ? dctx->propagation_wait_s : 90;
    ...
    {
        char loc2[8] = "";
        if (dns01_post(cl, challenge_url, "{}", nonce, sizeof(nonce), loc2, sizeof(loc2), &status,
                       &resp) < 0 ||
            (status != 200 && status != 202)) {
            log_error("acme_dns01", "respond to challenge returned %d: %s", status,
                      resp ? resp : "");
            goto done;
        }
        free(resp);
    ...
    /* ── 8. Delete TXT record ── */
    if (txt_created) {
        dctx->dns_ops->delete_txt(dctx->dns_ctx, dctx->zone_id[0] ? dctx->zone_id : "",
                                  txt_record_id);
        txt_created = 0;
    ...
    /* ── 10. Finalize order ── */
    {
        ...
        char loc2[8] = "";
        if (dns01_post(cl, finalize_url, pay, nonce, sizeof(nonce), loc2, sizeof(loc2), &status,
                       &resp) < 0 ||
            (status != 200 && status != 201)) {
            log_error("acme_dns01", "finalize returned %d: %s", status, resp ? resp : "");
            goto done;
        }
        free(resp);
        resp = NULL;
    ...
    /* ── 12. Re-fetch order to get certificate URL ── */
    {
        if (dns01_get_nonce(cl, nonce, sizeof(nonce)) < 0) goto done;
        char loc2[8] = "";
        if (dns01_post(cl, order_url, NULL /* POST-as-GET */, nonce, sizeof(nonce), loc2,
                       sizeof(loc2), &status, &resp) < 0 ||
            status != 200) {
    ...
    dns01_json_get_str(resp ? resp : "", "certificate", cert_url, sizeof(cert_url));
    free(resp);
    resp = NULL;
    ...
    {
        char loc2[8] = "";
        if (dns01_post(cl, cert_url, NULL /* POST-as-GET */, nonce, sizeof(nonce), loc2,
                       sizeof(loc2), &status, &resp) < 0 ||
            status != 200) {
    ...
    /* ── 14. Save cert.pem and key.pem to storage_path/domain/ ── */
    {
        ...
        /* cert.pem */
        char cert_path[4096];
        snprintf(cert_path, sizeof(cert_path), "%s/%s/cert.pem", cl->storage_path, domain);
        ...
        /* key.pem — mode 0600 */
        char key_path[4096];
        snprintf(key_path, sizeof(key_path), "%s/%s/key.pem", cl->storage_path, domain);
...
static int dns01_provider_init(void **provider_ctx, const struct vortex_config *cfg)
{
    ...
    strncpy(dctx->client.directory_url, ac->directory_url, sizeof(dctx->client.directory_url) - 1);
    strncpy(dctx->client.account_key_path, ac->account_key_path,
            sizeof(dctx->client.account_key_path) - 1);
    strncpy(dctx->client.storage_path, ac->storage_path, sizeof(dctx->client.storage_path) - 1);
    strncpy(dctx->client.email, ac->email, sizeof(dctx->client.email) - 1);
    ...
    if (strcmp(ac->dns_provider, "cloudflare") == 0 || ac->dns_provider[0] == '\0') {
...
static int dns01_provider_obtain(void *provider_ctx, const char *domain, struct cert_result *out)
{
    return acme_obtain_dns01((struct acme_dns01_ctx *)provider_ctx, domain, out);
}

static int dns01_provider_renew(void *provider_ctx, const char *domain, struct cert_result *out)
{
    return acme_obtain_dns01((struct acme_dns01_ctx *)provider_ctx, domain, out);
}
...
static void dns01_provider_destroy(void *provider_ctx)
{
    ...
    if (!dctx) return;
    acme_client_destroy(&dctx->client);
    if (dctx->dns_ops && dctx->dns_ctx) dctx->dns_ops->destroy(dctx->dns_ctx);
    free(dctx);
}
...
int acme_obtain_dns01(struct acme_dns01_ctx *ctx, const char *domain, struct cert_result *out)
{
    (void)ctx;
    (void)domain;
    (void)out;
...
static int dns01_provider_init_stub(void **ctx, const struct vortex_config *cfg)
{
    (void)ctx;
    (void)cfg;
    return -1;
...
static int dns01_provider_obtain_stub(void *ctx, const char *domain, struct cert_result *out)
{
    (void)ctx;
    (void)domain;
    (void)out;
...
static int dns01_provider_renew_stub(void *ctx, const char *domain, struct cert_result *out)
{
    (void)ctx;
    (void)domain;
    (void)out;
...
static void dns01_provider_destroy_stub(void *ctx)
{
    (void)ctx;
}
